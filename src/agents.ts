...
export class Expert {
  ...
  private system_prompt = `
    ...
    if you are explaining it to a beginner in Nuxt, but keep it concise. You will also need to harnass all your Vue.js skills.
  `;
  ...
  async getAnswers(questions: string[]) {
    const answers = await Promise.all(questions.map(async (question) => {
      const context = await queryDatabase(
        await getEmbeddings(this.llm, question),
        this.dbClient,
      );
...
export class Coder {
  ...
  CLI commands, code snippets, and explanations to help them with their project. The developer will ask you a
  question and along with the developers question, the opinion of a Nuxt.js expert will be added.
  ...
  the expert. If the developer asks an unrelated question, respond with "Let's focus on Nuxt.js". Do not hallucinate.
  `;
  ...
  private async generateQuestions(prompt: string) {
    ...
    Reply only with the format below to collect more info.
  ...
  private async generateFinalOutput(prompt: string, context: string) {
    const response = await this.llm.chat.completions.create({
      model: 'gpt-4-turbo',
      messages: [
        { role: 'system', content: this.system_prompt },
  ...
  async getResponse(prompt: string) {
    ...
    if (questions === undefined) {
      console.warn(`Failed to generate questions for prompt, response may be inaccurate.`);
    }

    const answers = await this.expert.getAnswers(questions ?? []);
    const expertOpinion = answers.join('\n\n');
    return await this.generateFinalOutput(prompt, expertOpinion);
  }
}
